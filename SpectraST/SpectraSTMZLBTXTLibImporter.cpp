...
/* Class: SpectraSTMZLBTXTLibImporter
 * 
 * Implements a library importer for the .mzlb.txt file format (used by NIST).
 * 
 */
...
SpectraSTMZLBTXTLibImporter::SpectraSTMZLBTXTLibImporter(vector<string>& impFileNames, SpectraSTLib* lib, SpectraSTCreateParams& params) :
...
// readFromCurFile - reads one .mzlb.txt file
void SpectraSTMZLBTXTLibImporter::readFromFile(string& impFileName) {
  
  ifstream fin;

  if (!myFileOpen(fin, impFileName)) {
    g_log->error("MZLBTXT IMPORT", "Cannot open " + impFileName + " for reading MZLBTXT-format (NIST) library");
    return;
  }
  
  g_log->log("MZLBTXT IMPORT", "Importing .mzlb.txt file \"" + impFileName + "\"."); 
  
  string line("");
  string::size_type pos = 0;
  
  if (g_verbose) {
    cout << "\nImporting spectra from .mzlb.txt library file..." << endl;
  } 
  
  // start the progress count
  ProgressCount pc(!g_quiet && !g_verbose, 1000);
  pc.start("\nImporting spectra from .mzlb.txt library file");
  ...
  while (true) {
	...
	// reach here because this .mzlb.txt probably is converted from a .sptxt,
	...
	break;	
        ...
        g_log->error("MZLBTXT IMPORT", "Badly formatted .mzlb.txt file! Library creation truncated at point of error.");
        pc.done();
        return;
	...
	// TODO: Put other useful information into comments!
        ...
        //        g_log->log("MZLBTXT IMPORT", "Unrecognized header field \"" + line + "\" ignored.");
      }
    }
    if (line == "_EOF_") {
	...
	comments += cess.str();
      ...
      Peptide* pep = createPeptide(fullName, charge, mods, fullName, "MZLBTXT");
	...
	if (m_params.setDeamidatedNXST) {
	  ...
	  entry->setOneComment("Mods", pep->MZLBTXTMods());
    ...
    while (nextLine(fin, line, "Name: ", "")) {
      if (line == "_EOF_") {
        ...
        if (entry && insertOneEntry(entry, "MZLBTXT")) {
          pc.increment();
          m_count++;	
        }
	...
	countss << m_numSkipped << " spectra skipped.";
        g_log->log("MZLBTXT IMPORT", countss.str());
        pc.done();
        return;
  ...
  g_log->log("MZLBTXT IMPORT", countss1.str());
  
  pc.done();
  
}

void SpectraSTMZLBTXTLibImporter::interpretPrecursorType(string precursorType, string& fullName) {
...
string SpectraSTMZLBTXTLibImporter::interpretInstrumentType(string instrumentType, string& comments) {
...
void SpectraSTMZLBTXTLibImporter::interpretSpectrumType(string spectrumType, string& comments) {
