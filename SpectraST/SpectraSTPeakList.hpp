...
public:
  ...
  template <class F>
  Peak* findPeakPtr(double mz, double tolerance, map<double, F>* foundMZs) {
  ...
  template <class F>
  Peak* findNearestPeakPtr(double mz, double tolerance, map<double, F>* foundMZs)  {
    ...
    // finds the nearest peak within (low, high)
    for (i = low; i != high; i++) {
      if (foundMZs->find(i->second.first->mz) == foundMZs->end() &&	
	  fabs(i->second.first->mz-mz) < fabs(max->second.first->mz-mz)) {
	max = i;
      }
    }
